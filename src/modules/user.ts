...
export default class User {
	...
	constructor(data: userData) {
		...
		this._nickname =
			`${data.login.charAt(0).toUpperCase()}${data.login.substr(1).toLowerCase()}`;
	...
	static getUserByLogin = (login: string) => User._users.find((user: User) =>
		user._login.toLowerCase() === login.toLowerCase());
